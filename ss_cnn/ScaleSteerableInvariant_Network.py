...
class Net_steerinvariant_mnist_scale(nn.Module):
    ...
    def forward(self, x):
        # if self.orderpaths == True:
        #print(x.shape)
        x = self.conv1(x) #print("layer1",x.shape)
        x = self.pool1(x)
        x = self.bn1(x)
        x = self.conv2(x) #print("layer2",x.shape)
        x = self.pool2(x)
        x = self.bn2(x)
        x = self.conv3(x) 
        xm = self.pool3(x) 
        ...
        return xm, embedding
    def parameters(self):
        return filter(lambda p: p.requires_grad, super(Net_steerinvariant_mnist_scale, self).parameters())
...
class Net_steerinvariant_cifar10_scale(nn.Module):
    def __init__(self):
        ...
        self.pool4 = nn.MaxPool2d(8,padding=2)
        self.bn4 = nn.BatchNorm2d(lays[3])
        self.pool5 = nn.MaxPool2d(8,padding=2)
        self.bn5 = nn.BatchNorm2d(lays[4])


        self.fc1 = nn.Conv2d(lays[3], 256, 1)
        self.fc1bn = nn.BatchNorm2d(256)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout2d(0.7)
        self.fc2 = nn.Conv2d(256, 10, 1)  # FC2
    ...
    def forward(self, x):
        ...
        return xm, embedding
    def parameters(self):
        return filter(lambda p: p.requires_grad, super(Net_steerinvariant_cifar10_scale, self).parameters())
...
class cnn_cifar10(nn.Module):
    def __init__(self,n):
        ...
        self.conv4 = nn.Conv2d(2*n, 4*n, 3, padding=1)
        self.conv5 = nn.Conv2d(4*n, 4*n, 3, padding=1)
        self.conv6 = nn.Conv2d(4*n, 8*n, 3, padding=1)
        
        self.conv7 = nn.Conv2d(8*n, 8*n, 3, padding=1)
        self.conv8 = nn.Conv2d(8*n, 16*n, 3, padding=1)
        
        self.fc = nn.Linear(16*n*4,10)
    ...
    def forward(self, x):
        ...
        return x, em
    def parameters(self):
        return filter(lambda p: p.requires_grad, super(cnn_cifar10, self).parameters())
...
class Net_steerinvariant_fmnist_scale(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool2(x)
        x = self.bn2(x)
        x = self.conv3(x) ; print("layer3",x.shape)
        xm = self.pool3(x)
        xm = self.bn3(xm)
        xm = xm.view([xm.shape[0], xm.shape[1] * xm.shape[2] * xm.shape[3], 1, 1])
        xm = self.fc1(xm)
