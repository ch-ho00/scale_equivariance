...
class Dconv2d_2(nn.Module):
    ...
    def forward(self, input):
        ...
        # Downscale
        downscale = [(self.downscale[d], self.downscale[d]) for d in range(len(self.downscale))]
        ...
        for s in range(sout):
            ...
            with torch.no_grad():
                x = funcs.convert(input[:,:,s:t,:,:],self.downscale[s:t]).reshape(input.size()[0],-1,input.size()[3],input.size()[4]).cuda()
            # Cut out the weights
            ...
            outputs.append(F.conv2d(x, w, bias=self.bias, stride=self.stride,padding=1)) #TODO: check padding
...
class DSS_ResBlock(nn.Module):
    ...
    def __init__(self, num_filters, channels_in, base, io_scales, skip,stride=1, res_option='A', use_dropout=False):
        ...
        self.conv2 = Dconv2d(num_filters, num_filters, [3,3,3], base, io_scales,stride=1, padding=1)
